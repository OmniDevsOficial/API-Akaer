        ...
        <>
            {revisoes.map((rev, idx) => (
                ...
                >
                    ...
                    <td className="px-3 py-3">
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium border ${statusBadgeClass(rev.status)}`}>
                            <span className={`inline-block w-2 h-2 rounded-full ${statusColorClass(rev.status)}`} />
                            <span className="leading-none text-sm">{rev.status}</span>
                        </span>
                    </td>
                    <td className="px-3 py-3">
                        {rev.arquivo ? (
                            <button onClick={() => onAbrirPdfRevisao(rev)} title="Visualizar PDF"
                    ...
                    {isAdmin && <td className="px-0 py-3 text-sm text-gray-400">-</td>}
                    <td className="px-3 py-3">
                        <button className="flex items-center gap-1 text-sm text-gray-400 hover:text-red-akaer transition-colors whitespace-nowrap"
...
function NormaCard({ norma, isAdmin, onAbrirPdf, onEditar, onAtualizarRevisao }: NormaCardProps) {
    const navigate = useNavigate();
    const orgao = norma.orgao_emissor?.nome || norma.orgao_emissor_id?.nome;
    const categoria = norma.categoria?.nome || norma.categoria_id?.nome;

    return (
        <div className="bg-white border border-font-border rounded-xl p-4">
            <div className="flex justify-between items-start mb-2">
                <span className="text-xs font-bold text-red-akaer tracking-wide">{norma.codigo_base || norma.codigo}</span>
                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium border ${statusBadgeClass(norma.status)}`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${statusColorClass(norma.status)}`} />
                    {norma.status}
                </span>
            </div>

            <p className="text-sm font-medium text-gray-900 leading-snug mb-3">{norma.titulo}</p>

            <div className="flex flex-wrap gap-1.5 mb-3">
                {orgao && <span className="text-xs text-gray-500 bg-gray-100 rounded px-2 py-1">{orgao}</span>}
                {categoria && <span className="text-xs text-gray-500 bg-gray-100 rounded px-2 py-1">{categoria}</span>}
                {norma.revisao && <span className="text-xs text-gray-400 bg-gray-50 rounded px-2 py-1">Rev. {norma.revisao}</span>}
            </div>
                ...
                {isAdmin && (
                    <>
                        <button onClick={() => onEditar(norma)}
                            className="flex items-center gap-1.5 text-xs text-gray-600 hover:text-red-akaer transition-colors min-h-[36px] py-1 ml-auto">
                            <Pencil size={14} /><span>Editar</span>
                        </button>
                        <button onClick={() => onAtualizarRevisao(norma)}
                            className="flex items-center gap-1.5 text-xs text-gray-600 hover:text-red-akaer transition-colors min-h-[36px] py-1">
                            <RefreshCw size={14} /><span>Revisão</span>
                        </button>
                    </>
                )}
            </div>
...
function NormaRow({ norma, isAdmin, onAbrirPdf, onAbrirPdfRevisao, onEditar, onAtualizarRevisao }: NormaRowProps) {
    ...
    const toggleAccordion = () => {
        if (temRevisoes) setAccordionAberto((prev) => !prev);
    };
    ...
    return (
        <>
            <tr
                ...
                className={`border-b border-font-border last:border-none hover:bg-red-50/60 transition-colors bg-white ${temRevisoes ? 'cursor-pointer' : ''} ${accordionAberto ? 'bg-red-50/40' : ''}`}
            >
                <td className="pl-4 pr-0 py-3">
                    {temRevisoes && (
                        <div title={accordionAberto ? 'Fechar histórico' : 'Ver histórico de revisões'}
                            ...
                            <ChevronRight size={16} className={`transition-transform duration-300 ease-in-out ${accordionAberto ? 'rotate-90' : 'rotate-0'}`} />
                        </div>
                    )}
                </td>
                ...
                <td className="px-3 py-3">
                    <button onClick={(e) => { e.stopPropagation(); onAbrirPdf(norma); }} title="Visualizar PDF"
                ...
                {isAdmin && (
                    <td className="px-0 py-4">
                        <button className="mb-[0.16rem] flex items-center gap-1 text-sm text-gray-700 hover:text-red-akaer transition-colors whitespace-nowrap"
                            onClick={(e) => { e.stopPropagation(); onEditar(norma); }}>
                            <Pencil size={14} /><span>Editar Norma</span>
                        </button>
                        <button className="flex items-center gap-1 text-sm text-gray-700 hover:text-red-akaer transition-colors whitespace-nowrap"
                            onClick={(e) => { e.stopPropagation(); onAtualizarRevisao(norma); }}>
                            <RefreshCw size={14} /><span>Atualizar Revisão</span>
                        </button>
                    </td>
                )}
                <td className="px-3 py-3">
                    <button className="flex items-center gap-1 text-sm text-gray-700 hover:text-red-akaer transition-colors whitespace-nowrap"
                        onClick={(e) => { e.stopPropagation(); navigate(`/normas/ver/${encodeURIComponent(norma.codigo)}`); }}>
                        <Eye size={14} /><span>Ver Norma</span>
                    </button>
                </td>
            </tr>
            {accordionAberto && temRevisoes && (
                <AccordionRevisoes revisoes={revisoes} onAbrirPdfRevisao={onAbrirPdfRevisao} isAdmin={isAdmin} />
            )}
        </>
    );
}
...
export default function TabelaNormas({
    ...
    const fetchNormas = useCallback(async () => {
        setCarregando(true);
        try {
            const data = await listarNormas({ page: 1, texto: searchText.trim(), filtros });
    ...
    const abrirPdfRevisao = async (revisao: RevisaoNorma) => {
        setNormaSelecionadaPdf({
            codigo: revisao.codigo, titulo: revisao.titulo, status: revisao.status,
            ...
            revisao: revisao.revisao, arquivo: revisao.arquivo ?? null,
        });
        setPdfModalAberto(true);
        try {
    ...
    const handleEditar = (norma: Norma) => {
        navigate(`/normas/editar/${encodeURIComponent(norma.codigo)}`, {
            state: {
                norma: {
                    id: norma.id, codigo: norma.codigo, titulo: norma.titulo,
                    revisao: norma.revisao, status: norma.status, arquivo: norma.arquivo,
                    orgaoEmissor: norma.orgao_emissor?.nome ?? norma.orgao_emissor_id?.nome,
                    categoria: norma.categoria?.nome ?? norma.categoria_id?.nome,
                }
            }
    ...
    const rodape = (
        <div className="px-4 py-3 border-t border-font-border">
    ...
    return (
        <>
            ...
            <div className="md:hidden flex flex-col gap-3 mt-4">
                {carregando ? (
                    <p className="text-sm text-gray-medium text-center py-6">Carregando normas...</p>
                ) : normasOrdenadas.length === 0 ? (
                    <p className="text-sm text-gray-medium text-center py-6">Nenhuma norma encontrada.</p>
                ) : (
            ...
            <div className="hidden md:block border border-font-border rounded-lg overflow-x-auto">
                <table className="w-full min-w-[920px]">
                    ...
                    <tbody>
                        {carregando ? (
                            <tr><td colSpan={totalColunas} className="px-6 py-6 text-sm text-gray-medium text-center">Carregando normas...</td></tr>
                        ) : normas.length === 0 ? (
                            <tr><td colSpan={totalColunas} className="px-6 py-6 text-sm text-gray-medium text-center">Nenhuma norma encontrada.</td></tr>
                        ) : (
                            normasOrdenadas.map((norma) => (
                                <NormaRow
                                    key={norma.id}
            ...
            <PdfViewerModal open={pdfModalAberto} onOpenChange={setPdfModalAberto} norma={normaSelecionadaPdf} />
            <UpdateVersionModal open={updateModalAberto} onOpenChange={setUpdateModalAberto} norma={normaSelecionadaUpdate} onSuccess={fetchNormas} />
        </>
    );
}